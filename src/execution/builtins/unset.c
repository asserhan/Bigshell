...
/*   Updated: 2023/05/24 19:42:34 by hasserao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int is_valid_arg(char *arg)
{
	...
	if (start_with(arg, alpha) == 0)
	{
		if (start_with(arg, "-"))
			...
			put_error_ex("minishell:  unset: ", arg, ": not a valid identifier\n", 1);
		return (1);
	}
	j = 1;
	while (arg[j])
...
void ft_unset(t_exec_context *exContext)
{
	...
	else
	{
		...
		while (exContext->cmds->args[i])
		{
			...
			{
				elem = search_env_elem(exContext->env, exContext->cmds->args[i]);
				if(!elem)
					return;
